...
geometry_msgs::Vector3 rej_force;

//pot fields constant
float k_linear_y = 2.0;
...
geometry_msgs::Twist calculate_speeds(float goal_x, float goal_y)
{
    ...
    if (distance > 0)
    {
        ...
        if (pot_fields)
        {
            result.linear.y = k_linear_y * rej_force.y; // Use the y component of the rejection force
        }
        else
        {
            result.linear.y = 0;
...
void callback_pot_fields(const geometry_msgs::Vector3::ConstPtr& msg)
{
    rej_force = *msg;
    std::cout << "rej_force: [" << rej_force.x << ", " << rej_force.y << ", " << rej_force.z << "]" << std::endl;
}
...
void callback_enable(const std_msgs::Bool::ConstPtr& msg)
{
    if (msg->data)
    {
        std::cout << "LegFinder.->Enable received" << std::endl;
        sub_legs_pose = n->subscribe("/hri/leg_finder/leg_pose", 1, callback_legs_pose);
        ...
        sub_pot_fields = n->subscribe("/navigation/obs_detector/pf_rejection_force", 1, callback_pot_fields);
    }
    else
    {
